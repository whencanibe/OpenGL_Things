...
void MyObject::Init(const char* filename)
{
	...
	for (int i = 0; i < fNum; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			int index = tri[i].vind[j];

			vec3 n = normalize(nor[index]); 
			float s = 0.5f + atan2(n.z, -n.x) / (2.0f * 3.141592); 
			float t = 0.5f - asin(n.y) / 3.141592;             
			
			v[3 * i + j].pos = pos[index];
			v[3 * i + j].nor = nor[index];
			v[3 * i + j].tex = vec2(s, 1-t); 
		}
	}
...
void MyObject::Draw(GLuint program)
{
	...
	glVertexAttribPointer(vTexCoord, 2, GL_FLOAT, GL_FALSE, sizeof(MyObjectVertex), BUFFER_OFFSET(sizeof(vec4) + sizeof(vec3)));
